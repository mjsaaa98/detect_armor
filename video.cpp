...
//        writer<<frame;     //写入视频文件
        ...
        else if(data.isfind == 1)
        {
            sum_yaw_offset += history_yaw_offset[0];
#ifdef SHOW_DEBUG
            cout<<"sum_offset:"<<sum_yaw_offset<<endl;
#endif
            if(ans.Rotated_SolveAngle(RRect,xAngle,yAngle,dis,camera_location,0,0,Point(0,0)))
            {
                //第一次接收数据标志
                if (isreceiveflag == 1)
                {
